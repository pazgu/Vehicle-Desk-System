...
export class NewRideComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.currentUserId) {
      this.rideUserChecksService
        ...
        .subscribe((result) => {
          ...
          if (this.currentUserBlocked) {
            ...
            const rebookData = nav?.extras?.state?.['rebookData'] as
              | RebookData
              | undefined;
      ...
      console.warn(
        'Current user ID not found in token during ngOnInit. Disabling request.'
      );
  ...
  private applyRebookData(data: RebookData): void {
    ...
    this.rideForm.patchValue({
      ...
      passengers:
        data.passengers_count ?? this.rideForm.get('passengers')?.value,
