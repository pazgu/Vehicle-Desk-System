...
export class UpdateLogComponent {
  ...
  getAuditRows() {
    ...
    switch (selectedLog.entity_type) {
      case 'User':
        return getUserAuditRows(
          selectedLog.change_data.old,
          selectedLog.change_data.new,
          this.departments
        );
      case 'Ride':
        return getRideAuditRows(
          selectedLog.change_data.old,
          selectedLog.change_data.new,
          this.cityMap,
          this.users,
      ...
      case 'Vehicle':
        return getVehicleAuditRows(
          selectedLog.change_data.old,
          selectedLog.change_data.new,
          this.getDeptName.bind(this)
        );
      case 'Department':
        return getDepartmentAuditRows(
          selectedLog.change_data.old,
          selectedLog.change_data.new,
          this.users
        );
