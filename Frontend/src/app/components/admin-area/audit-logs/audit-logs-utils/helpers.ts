...
export function getEnglishStatusLabel(log: any): string {
 ...
 export function translateFuelType(fuelType: string | null | undefined): string {
  ...
  export function translateVehicleStatus(status: string | null | undefined): string {
    if (!status) return '';
    switch (status.toLowerCase()) {
      case 'available':
        return 'זמין';
  ...
  export function translateFreezeReason(freezeReason: string | null | undefined): string {
  ...
  export function getVehicleAuditRows(oldData: any, newData: any, getDepartmentNameById: (id: string | null | undefined) => string): Array<{ label: string, oldValue: any, newValue: any }> {
    return [
      ...
      { label: 'סוג דלק', oldValue: translateFuelType(oldData.fuel_type), newValue: translateFuelType(newData.fuel_type) },
      { label: 'סטטוס', oldValue: translateVehicleStatus(oldData.status), newValue: translateVehicleStatus(newData.status) },
      { label: 'שימוש אחרון', oldValue: oldData.last_used_at, newValue: newData.last_used_at }, // <-- Added this line
      { label: 'סיבת הקפאה', oldValue: translateFreezeReason(oldData.freeze_reason), newValue: translateFreezeReason(newData.freeze_reason) },
      { label: 'פרטי הקפאה', oldValue: oldData.freeze_details, newValue: newData.freeze_details },
      { label: 'מחלקה', oldValue: getDepartmentNameById(oldData.department_id), newValue: getDepartmentNameById(newData.department_id) },
...
export function getCityName(cityId: string, cityMap: Record<string, string>): string {
  return cityMap[cityId] || cityId;
}

export function formatRouteFromChangeData(changeData: any, cityMap: Record<string, string>): string {
  const start = changeData.start_location;
  const stop = changeData.stop;
  const destination = changeData.destination;
  const extraStops = changeData.extra_stops; // array or undefined
 ...
 export function getUserAuditRows(oldData: any, newData: any,departments: {
    id: string;
    name: string;
    ...
    return [
      ...
      { label: 'תפקיד', oldValue: translateUserRole(oldData.role), newValue: translateUserRole(newData.role) },
      { label: 'מזהה עובד', oldValue: oldData.employee_id, newValue: newData.employee_id },
      { label: 'מחלקה', oldValue: getDepartmentNameById(oldData.department_id,departments), newValue: getDepartmentNameById(newData.department_id,departments) },
  ...
  export function formatRoute(start: string, stop: string, extraStops: string[] | undefined, destination: string,cityMap: Record<string, string>): string {
  const allStops = [start, stop, ...(extraStops || []), destination];
  ...
  export function getRideAuditRows(oldData: any, newData: any,cityMap: Record<string, string>,
    users: {
    id: string;
    first_name: string;
    last_name: string;
    ...
    return [
      ...
      {
        label: 'מסלול',
        oldValue: formatRoute(oldData.start_location, oldData.stop, oldData.extra_stops, oldData.destination,cityMap),
        newValue: formatRoute(newData.start_location, newData.stop, newData.extra_stops, newData.destination,cityMap)
      },
      { label: 'סוג נסיעה', oldValue: translateRideType(oldData.ride_type), newValue: translateRideType(newData.ride_type) },
      { label: 'סיבת בחירה ברכב 4X4', oldValue: oldData.four_by_four_reasonn, newValue: newData.four_by_four_reasonn },
      { label: 'שם משתמש', oldValue: getUserFullNameById(oldData.user_id,users), newValue: getUserFullNameById(newData.user_id,users) },
      { label: 'שם משתמש עוקף', oldValue: getUserFullNameById(oldData.override_user_id,users), newValue: getUserFullNameById(newData.override_user_id,users) },
      { label: 'לוחית רישוי', oldValue: getPlateNumber(oldData.vehicle_id,vehicles), newValue: getPlateNumber(newData.vehicle_id,vehicles) },
      { label: 'דגם רכב', oldValue: getVehicleModel(oldData.vehicle_id,vehicles), newValue: getVehicleModel(newData.vehicle_id,vehicles) },
      { label: 'סטטוס', oldValue: translateRideStatus(oldData.status), newValue: translateRideStatus(newData.status) },
  ...
  export function getDepartmentAuditRows(oldData: any, newData: any,users: {
    id: string;
    first_name: string;
    last_name: string;
    user_name: string;
    ...
    return [
      ...
      { label: 'שם מנהל מחלקה', oldValue: getUserFullNameById(oldData?.supervisor_id,users), newValue: getUserFullNameById(newData?.supervisor_id,users) },
    ];
  }
