...
import { getRowBackgroundColor, getEnglishStatusLabel } from '../audit-logs-utils/helpers'; // adjust import

export async function exportToPDF(filteredLogs: any[]) {
    const weeklyLogs = filteredLogs; // already resolved array
...
export async function exportToCSV(filteredLogs: any[]) {
  ...
  const csv = Papa.unparse({ fields: [], data: [...legendRows, headers, ...dataRows] });
  ...
  fullSheet.forEach((row, rowIndex) => {
    row.forEach((cell, colIndex) => {
      ...
      if (rowIndex === legendLength) {
        wsData[cellRef].s = { font: { bold: true }, fill: { fgColor: { rgb: 'E0E0E0' } } };
        return;
      }

      if (rowIndex > 0 && rowIndex < legendLength && colIndex === 0) {
        wsData[cellRef].s = { fill: { fgColor: { rgb: legendColors[rowIndex].replace('#', '') } } };
        return;
      }
      ...
      if (rowIndex > legendLength) {
        ...
        wsData[cellRef].s = { fill: { fgColor: { rgb: bgColor.replace('#', '') } } };
      }
    });
  });
